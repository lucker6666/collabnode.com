...
angular.module('collabApp.directives', []).directive('autoComplete', function($timeout) {
...
angular.module('collabApp.services', ['ngResource'])
  .value('version', '0.1')
...
$(function ($, _, Backbone, io) {
  ...
  AppView = Backbone.View.extend({
    ...
    render: function () {
      ...
      {
        var uid = this.uids[i];
        var done = Todos.filterbyOwner(uid,Todos.done()).length
        ,remaining = Todos.filterbyOwner(uid,Todos.remaining()).length;
        footcontent[uid] = this.statsTemplate({done: done, remaining: remaining});
        this.$("#main-"+uid).hide();
